...
const LessonsNew: React.FC = () => {
    ...
    const filteredLessonsByAlbum = useMemo(() => lessons.filter(lesson => lesson.album === chosenAlbum), [lessons, chosenAlbum]);

    const renderedLessons = useMemo(() => lessons.map((lesson) => {
        if (lesson.album === chosenAlbum)
            return (
                <div key={lesson.id} className="lessonItem" onClick={() => handleLessonClick(lesson.url, lesson.id)}>
